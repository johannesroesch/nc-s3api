...
class SigV4ParserTest extends TestCase {
	...
	public function testFromHeaderStripsLeadingWhitespaceFromHeader(): void {
		$header = '  AWS4-HMAC-SHA256 Credential=AKID/20240101/us-east-1/s3/aws4_request,'
				. 'SignedHeaders=host,'
				. 'Signature=sig';
		$p = SigV4Parser::fromHeader($header);
	...
	public function testFromHeaderPayloadParsedCorrectly(): void {
		// If substr offset were +0 instead of +1, the space would bleed into the
		// first key name and Credential would not be found → exception.
		// This test therefore distinguishes +1 from +0.
		$header = 'AWS4-HMAC-SHA256 Credential=AKID/20240101/us-east-1/s3/aws4_request,'
				. 'SignedHeaders=host,'
				. 'Signature=sig';
		$p = SigV4Parser::fromHeader($header);
		// If +0 were used, accessKey would start with a space or parsing would fail
		$this->assertSame('AKID', $p->accessKey);
		$this->assertSame('us-east-1', $p->region);
	}
	public function testFromHeaderAlgorithmWithoutSpaceThrows(): void {
		// Header starts with algorithm string but no trailing space → must throw
		$this->expectException(S3Exception::class);
		SigV4Parser::fromHeader('AWS4-HMAC-SHA256Credential=AKID/20240101/us-east-1/s3/aws4_request,SignedHeaders=host,Signature=sig');
	}

	public function testFromHeaderSignedHeadersTrimmed(): void {
		// Spaces around semicolons in SignedHeaders must be stripped
		$header = 'AWS4-HMAC-SHA256 Credential=AKID/20240101/us-east-1/s3/aws4_request,'
	...
	public function testFromHeaderMissingEqualsInPartYieldsEmptyValue(): void {
		// A part without '=' should not crash — array_pad fills $v with ''
		// The Signature ends up empty → must throw
		$this->expectException(S3Exception::class);
		SigV4Parser::fromHeader('AWS4-HMAC-SHA256 Credential=AKID/20240101/us-east-1/s3/aws4_request,SignedHeaders=host,Signature');
	}

	// -------------------------------------------------------------------------
	// fromQueryParams — additional coverage
	// -------------------------------------------------------------------------

	public function testFromQueryParamsSignedHeadersTrimmed(): void {
		$q = [
			'X-Amz-Algorithm' => 'AWS4-HMAC-SHA256',

...
final class OperationResolver {
    ...
    private function resolvePutBucket(array $q): string {
        if (isset($q['versioning']))  return self::PUT_BUCKET_VERSIONING;
        if (isset($q['tagging']))     return self::PUT_BUCKET_TAGGING;
        if (isset($q['acl']))         return self::PUT_BUCKET_ACL;
        if (isset($q['cors']))        return self::PUT_BUCKET_CORS;
        if (isset($q['encryption']))  return self::PUT_BUCKET_ENCRYPTION;
        return self::CREATE_BUCKET;
    }

    private function resolveGetBucket(array $q): string {
        if (isset($q['versioning']))      return self::GET_BUCKET_VERSIONING;
        if (isset($q['tagging']))         return self::GET_BUCKET_TAGGING;
        if (isset($q['acl']))             return self::GET_BUCKET_ACL;
        if (isset($q['cors']))            return self::GET_BUCKET_CORS;
        if (isset($q['encryption']))      return self::GET_BUCKET_ENCRYPTION;
        if (isset($q['location']))        return self::GET_BUCKET_LOCATION;
        if (isset($q['versions']))        return self::LIST_OBJECT_VERSIONS;
        if (isset($q['uploads']))         return self::LIST_MULTIPART_UPLOADS;
        // list-type=2 → ListObjectsV2 (also default for modern clients)
        if (($q['list-type'] ?? '') === '2') return self::LIST_OBJECTS_V2;
        return self::LIST_OBJECTS;
    }

    private function resolveDeleteBucket(array $q): string {
        if (isset($q['tagging']))    return self::DELETE_BUCKET_TAGGING;
        if (isset($q['cors']))       return self::DELETE_BUCKET_CORS;
        if (isset($q['encryption'])) return self::DELETE_BUCKET_ENCRYPTION;
    ...
    private function resolveGetObject(S3Request $request): string {
        $q = $request->queryParams;
        if (isset($q['tagging']))    return self::GET_OBJECT_TAGGING;
        if (isset($q['acl']))        return self::GET_OBJECT_ACL;
        if (isset($q['attributes'])) return self::GET_OBJECT_ATTRIBUTES;
        if (isset($q['uploadId']))   return self::LIST_PARTS;
    ...
    private function resolvePutObject(S3Request $request): string {
        ...
        if (isset($q['uploadId'], $q['partNumber'])) return self::UPLOAD_PART;
        if (isset($q['tagging'])) return self::PUT_OBJECT_TAGGING;
        if (isset($q['acl']))     return self::PUT_OBJECT_ACL;
        // CopyObject is indicated by the x-amz-copy-source header
        if ($request->getHeader('x-amz-copy-source') !== '') return self::COPY_OBJECT;
        return self::PUT_OBJECT;
    }

    private function resolveDeleteObject(array $q): string {
        if (isset($q['tagging']))  return self::DELETE_OBJECT_TAGGING;
        if (isset($q['uploadId'])) return self::ABORT_MULTIPART_UPLOAD;
        return self::DELETE_OBJECT;
    }

    private function resolvePostObject(array $q): string {
        if (isset($q['uploads']))  return self::INITIATE_MULTIPART_UPLOAD;
        if (isset($q['uploadId'])) return self::COMPLETE_MULTIPART_UPLOAD;
        if (isset($q['restore']))  return self::RESTORE_OBJECT;
